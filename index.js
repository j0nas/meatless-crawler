...
const whitelistInclude = ["vegan", "vegetarian", "meat"];
  ...
  callback: (err, res, done) => {
    if (err) {
      ...
      cheerio(links).each((i, link) => {
        ...
        if (
          href &&
          (isIncludedInList(href, whitelistInclude) ||
            isIncludedInList(cheerio(link).text(), whitelistInclude)) &&
          !isIncludedInList(href, blacklistWords)
        ) {
          const hrefUrl = href.startsWith("/")
            ? URL.resolve(res.request.uri.href, href)
            : href;
...
process.on("SIGINT", async () => {
  shouldWrite = false;
  await Promise.all(
    Object.keys(visitedDictionary).map(site =>
      write(`./urls/${site}.json`, visitedDictionary[site], { spaces: 2 })
    )
  );
  process.exit();
});
